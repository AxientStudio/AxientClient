...
public class HudRenderer {
    ...
    public static void renderElement(String id, DrawContext ctx, MinecraftClient mc, int[] pos) {
        ...
        if (scale != 1.0f) {
            // In 1.21.8, DrawContext.getMatrices() returns Matrix3x2fStack (no push/pop, 2 args)
            // Scale around pos origin using translate + scale (2D only)
    ...
    public static void renderCoords(DrawContext ctx, MinecraftClient mc, ClientPlayerEntity p, int[] pos) {
        ...
        int biomeOffset = mc.textRenderer.getWidth("Biome: ");
        ctx.drawText(mc.textRenderer, "§7Biome: ", x, y + 30, 0xFFFFFFFF, true);
        ctx.drawText(mc.textRenderer, biome,        x + biomeOffset, y + 30, biomeColor, true);
    }
    ...
    public static void renderTotemAt(DrawContext ctx, MinecraftClient mc, ClientPlayerEntity p, int[] pos) {
        ...
        int col = own == 0 ? 0xFFFF4444 : own == 1 ? 0xFFFFAA00 : 0xFF55FF55;
        ctx.drawText(mc.textRenderer, "§6⊕ Own: §f" + own + "  §c☠ Enemy: §f" + TotemCounter.getEnemyTotemsUsed(),
                pos[0], pos[1], col, true);
    }
    ...
    public static void renderSprintStatus(DrawContext ctx, MinecraftClient mc) {
        ...
        ctx.drawText(mc.textRenderer, SneakSprintToggle.getStatusText(), barX, 18, 0xFFFFFFFF, true);
    }

    // ── Inventory Hub (27 slots, skip hotbar) ──
