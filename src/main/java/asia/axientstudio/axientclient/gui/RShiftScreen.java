...
public class RShiftScreen extends Screen {
    ...
    private static final Identifier LOGO =
            Identifier.of("axientclient", "textures/logo_nobackground.png");
    ...
    private int hubBtnX, hubBtnY;
    private final int hubBtnW = 160, hubBtnH = 24;
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("§aSave & Exit"),
                btn -> saveAndExit()
        ).dimensions(this.width - 98 - gap, by, 98, bh).build());

        addDrawableChild(ButtonWidget.builder(Text.literal("§eTop-Left"),
                btn -> preset(0)
        ).dimensions(gap, by, 78, bh).build());

        addDrawableChild(ButtonWidget.builder(Text.literal("§eTop-Right"),
                btn -> preset(1)
        ).dimensions(gap + 82, by, 82, bh).build());

        addDrawableChild(ButtonWidget.builder(Text.literal("§eCompetitive"),
                btn -> preset(2)
        ).dimensions(gap + 168, by, 94, bh).build());

        addDrawableChild(ButtonWidget.builder(Text.literal("§cReset Scale"),
                btn -> { if (selectedId != null) hud.setScale(selectedId, 1f); }
        ).dimensions(gap + 266, by, 90, bh).build());
    }
    ...
    private int[] scaledSize(String id) {
        int[] b = BASE_SIZES.get(id);
        float s = hud.getScale(id);
        return new int[]{(int)(b[0]*s), (int)(b[1]*s)};
    }

    private boolean hit(String id, double mx, double my) {
        int[] pos = hud.getPos(id);
        int[] sz  = scaledSize(id);
        int pad = 4;
        return mx >= pos[0]-pad && mx <= pos[0]+sz[0]+pad
            && my >= pos[1]-pad && my <= pos[1]+sz[1]+pad;
    }
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        // Logo — drawGuiTexture is the correct API in 1.21.8 (drawTexture was removed)
        // RenderPipelines is in net.minecraft.client.gl
        ctx.drawGuiTexture(
                net.minecraft.client.gl.RenderPipelines.GUI_TEXTURED,
                LOGO, logoX, logoY, logoW, logoH
        );

        // Open Hub button
        boolean hubHov = mouseX >= hubBtnX && mouseX <= hubBtnX+hubBtnW
                      && mouseY >= hubBtnY && mouseY <= hubBtnY+hubBtnH;
        ...
        // Status
        String status = "";
        if (hud.draggingElement != null) {
            int[] p = hud.getPos(hud.draggingElement);
            status = "§7Moving §f" + hud.draggingElement + " §7→ " + p[0] + ", " + p[1];
        } else if (selectedId != null) {
            status = "§7Selected: §f" + selectedId + "  §e" + (int)(hud.getScale(selectedId)*100) + "% §7(scroll=resize, R=reset)";
        } else if (hoveredId != null) {
            status = "§7Hover: §f" + hoveredId + " §7— click to select";
        }
        if (!status.isEmpty())
            ctx.drawCenteredTextWithShadow(mc.textRenderer, status, this.width/2, this.height-34, 0xFFFFFF);
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        // Open Hub button
        if (button == 0 && mx >= hubBtnX && mx <= hubBtnX+hubBtnW
                        && my >= hubBtnY && my <= hubBtnY+hubBtnH) {
            hud.savePositions();
            hud.dragMode = false;
    ...
    public boolean mouseDragged(double mx, double my, int button, double dx, double dy) {
        if (button == 0 && hud.draggingElement != null) {
            ...
            int[] pos = hud.getPos(hud.draggingElement);
            int[] sz  = scaledSize(hud.draggingElement);
            int sx = Math.round(pos[0]/(float)SNAP)*SNAP;
            int sy = Math.round(pos[1]/(float)SNAP)*SNAP;
            hud.setPos(hud.draggingElement,
                    Math.max(0, Math.min(this.width-sz[0], sx)),
                    Math.max(12, Math.min(this.height-sz[1]-28, sy)));
            return true;
        }
